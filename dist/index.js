...
if (options !== false) {
    ...
    if (!nodeModules || nodeModules.length == 0) {
        ...
        const deleteModules = await askUser(`${colorizeTexts(colors.green, String(nodeModules?.length))} node_modules found. Do you want to delete them? ${options.force
            ? colorizeTexts(colors.red, "Since the --force flag is used, the check step will be omited")
            : ""}`);
        if (!deleteModules) {
